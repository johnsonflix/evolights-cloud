-- 000001_initial.sql
-- EvoLights cloud schema (initial). Mirrors the legacy schema.sql so installs that
-- previously bootstrapped via /docker-entrypoint-initdb.d remain compatible. The
-- runner records this migration as applied even on already-bootstrapped DBs by
-- using `if not exists` everywhere.
